...
.. code-block:: python
    ...
    # a ?? b
    _t if ((_t := a) is not None) else None
...
The "``None`` coalescing" operator can bridge that gab by adding an
...
The result is stored in a temporary variable, so that the expression
is not evaluated again. If the value is ``not None``, it is returned.
If it is ``None``, the right hand side expression is evaluated and
returned instead.
...
operators ``?.`` and ``?[ ]``. As the "``None`` coalescing" and
"coalescing assignment" operators have their own use cases, the
``None``-aware access operators were moved into a separate
document, see PEP-XXX. Both proposals can be adopted independently
of one another.
...
In comparison and new (soft-) keyword would likely not enjoy the same
...
Furthermore, setting te precedence between ``or`` and conditional
...
- Clear control flow, no more ``if ... is None else ...`` and the
  inverse ``if ... is not None else ...`` in the same code blocks.
